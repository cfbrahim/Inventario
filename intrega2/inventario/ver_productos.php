            ...
            <tbody>
                ...
                if ($stmt->rowCount() > 0) {
                    while($fila = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        if (!empty($fila['img_prod']) && file_exists($_SERVER['DOCUMENT_ROOT'] . trim($fila['img_prod']))) {
                            echo "<img src='".htmlspecialchars(trim($fila['img_prod']))."' style='width:50px;height:50px;object-fit:cover;' alt='".htmlspecialchars($fila['nombre_producto'])."'>";
                        } else {
